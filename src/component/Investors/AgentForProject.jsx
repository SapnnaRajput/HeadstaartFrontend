...
const AgentForProject = () => {
  ...
  const fetchAssistance = async () => {
    ...
    try {
      ...
      if (response?.data?.status) {
        setAgents(response?.data?.data.agent_without_project);
      } else {
  ...
  const handleAgentClick = (agent) => {
    navigate(`/investor/agent-for-project/${agent?.agent_unique_id}/${agent?.chat_initiate_id}`)
    // setOpenModal(true);
          ...
          {agents.length > 0 ? (
           ...
           {agents.map((agent) => (
             ...
             >
               ...
               <div className="flex flex-col items-center p-4 text-center h-full">
                 <div className="w-full mb-2">
                   <h2 
                     className="font-bold text-base text-[#05004E] truncate max-w-full capitalize cursor-pointer" 
                     onClick={() => handleAgentClick(agent)}
                   >
                 ...
                 <div className="w-full mb-4">
                   <span className="text-sm text-gray-500 block truncate capitalize">
                     {agent.title}
                   </span>
                 </div>
                 ...
                 <div className="w-full mt-auto">
                   <button
                     ...
                     className="w-full py-2 px-4 bg-[#4A3AFF] text-white rounded-lg hover:bg-[#3D32CC] transition-colors flex items-center justify-center gap-2"
                   >
      ...
      {openModal && singleAgent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative">
            ...
            {/* Project Section - Only show if project data exists */}
                  ...
                  <div className="rounded-lg border overflow-hidden">
                    ...
                    <div className="p-4">
                      <h4 className="font-medium mb-1 capitalize">
                        {singleAgent.title || "Project Title"}
                      </h4>

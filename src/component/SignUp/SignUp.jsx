...
const SignupForm = () => {
  ...
  const validateForm = () => {
    ...
    if (isStudent && type === "entrepreneur") {
      if (!formData.school_university) {
        notify("error", "School/University is required");
        return false;
      }
      if (!formData.student_identity) {
        notify("error", "Student ID is required");
        return false;
      }
      if (!studentIdFile) {
        notify("error", "Please upload your student ID file");
        return false;
      }
    } else {
    ...
    if (!formData.house_no) {
      notify("error", "House number is required");
      return false;
    }
    ...
    if (!selectedCountry || !selectedState || !selectedCity) {
      notify("error", "Please select your country, state, or city");
      return false;
    }
    ...
    if (!agreeToTerms) {
      notify("error", "Please agree to the Terms of Service");
      return false;
    }
    if (type === "agent") {
      if (!agentInputPercentage) {
        notify("error", "Agent percentage is required");
        return false;
      }
      if (parseFloat(agentInputPercentage) > parseFloat(agentPercentage)) {
        notify("error", `Agent percentage cannot exceed ${agentPercentage}%`);
        return false;
      }
    }
    if (!isEmailVerified) {
  ...
  const handleSignupClick = async () => {
    ...
    try {
      ...
      // if (!coordinates) {
      //   notify("error", "Location not track try again later");
      //   return;
      // }
      ...
      submitData.append("role", type);
      submitData.append("house_no", formData.house_no);
      submitData.append("address", formData.address);
      submitData.append("country", selectedCountry.value);
      submitData.append("state", selectedState.value);
      submitData.append("city", selectedCity ? selectedCity.value : "");
      ...
      if (type === "entrepreneur") {
        submitData.append("student_entrepreneur", isStudent ? "Yes" : "No");
        submitData.append("school_university", formData.school_university);
        submitData.append("student_identity", formData.student_identity);
        if (studentIdFile) {
          submitData.append("student_id_file", studentIdFile);
        }
      }
      if (type === "agent") {
        submitData.append("agent_percentage", agentInputPercentage);
      }
                ...
                <div className="space-y-4">
                  ...
                  {isStudent && type === "entrepreneur" ? (
                    <>
                      ...
                      <div className="col-span-2">
                        <label className="block text-md mb-1">
                          Attach your Student ID
                          <span className="text-red-500">*</span>
                        </label>
                        <label className="relative border rounded py-3 px-3 text-center cursor-pointer hover:bg-gray-50 flex items-center justify-center gap-2 group">
                          <input
                            type="file"
                          ...
                          <p className="text-gray-500 text-md group-hover:text-blue-600 transition-colors">
                  ...
                  <LocationSelector
                    ...
                    labelClass="block text-md mb-1"
                    inputClass="text-md mb-2"
                  />
                  <div className="col-span-2">
                    <label className="block text-md mb-1">
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      className="w-full px-2.5 py-2 text-md border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                    />
